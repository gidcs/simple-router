...
/* LKS start */
/* to check if ip_dst is the ip of one interface of the router */
/* router can use this check if the packet's ip_dst is itself */
int sr_interface_match_ip(struct sr_instance* sr,uint32_t ip_dst){
	struct sr_if* if_walker;
	/* -- REQUIRES -- */
	assert(sr);
	if_walker = sr->if_list;
	while(if_walker){
		if(if_walker->ip==ip_dst) return 1;
		if_walker=if_walker->next;
	}
	return 0;
}
