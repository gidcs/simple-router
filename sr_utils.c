...
sr_icmp_hdr_t* get_icmp_header(sr_ip_hdr_t *ip_header){
	uint8_t* icmp_header;
	/* fprintf(stderr,"[LKS] (get_icmp_header) IP header len: %d\n",get_ip_header_len(ip_header));*/
	icmp_header = (uint8_t*)(ip_header);	
	icmp_header += get_ip_header_len(ip_header);
	return (sr_icmp_hdr_t*) icmp_header;
}
...
uint16_t get_arp_hrd(sr_arp_hdr_t *arp_header){
	return ntohs(arp_header->ar_hrd);
}
uint16_t get_arp_pro(sr_arp_hdr_t *arp_header){
	return ntohs(arp_header->ar_pro);
}
...
struct sr_rt* sr_longest_prefix_match(struct sr_instance* sr, struct in_addr addr){
	struct sr_rt* cur_entry;
	struct sr_rt* lpm_entry;
	...
	while(cur_entry) {
		/* network id = ip_addr & mask_addr					*/
		/* For example										*/
		/* 192.168.1.1   = 192.168.1.193 & 255.255.255.0	*/
		/* 192.168.1.192 = 192.168.1.193 & 255.255.255.192	*/ /* more strict so we choose this */
		/* note: in_addr.s_addr is a uint32_t value  */
		if((cur_entry->dest.s_addr & cur_entry->mask.s_addr)==(addr.s_addr & cur_entry->mask.s_addr)){
			/* if mask.s_addr of two dest is same, the interface should be same */
			/* equal or not is not important in here */
			if(lpm_len<cur_entry->mask.s_addr){
				lpm_len = cur_entry->mask.s_addr;
				lpm_entry = cur_entry;
			}
		}
